...
control MyIngress(inout headers hdr,
    ...
    // VLAN functionality is delegated to the egress.
    table switch_table {
        key = {
        ...
        actions = {
            forward;
            flood;      // For unknown destination MAC addresses
        }
...
control MyEgress(inout headers hdr,
    ...
    // to create a table.
    table vlan_table {
        // Match on egress port and VLAN ID
        key = {
            standard_metadata.egress_port: exact;
            meta.vid: exact;
        }
        // Let matching packets pass; if there is no match, drop
        actions = {
            drop;
            noop;
        }
